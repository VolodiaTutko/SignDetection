...
destination_folder = "./Photo_to_text"
sorted_files = sorted(os.listdir(destination_folder), key=lambda x: int(x.split('.')[0]))# НЕОБХІДНО ФОТО НАЗИВАТИ ЗА ЗРОСТАННЯМ
...
for img in sorted_files:
    ...
    frame = cv2.imread(os.path.join(destination_folder, img))
